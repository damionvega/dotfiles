...
set BREW_GREP (echo $PATH | grep $BREW_PATH)

if not string match -e $BREW_PATH $BREW_GREP
...
set FISH_GREP (cat /etc/shells | grep $FISH_PATH)

if not string match -e $FISH_PATH $FISH_GREP
